...
const times = { dawn: 5 * 60, sunrise: 6 * 60 + 30, sunset: 19 * 60 + 30, dusk: 21 * 60 }

const slices = await Promise.all(
  Array.from({ length: HOURS }, (_, h) => generatePng(getSkyColors(times, h * 60), SLICE_W, SLICE_H)),
)

const labels = Array.from({ length: HOURS }, (_, h) =>
...
const out = await sharp({
  create: { width: WIDTH, height: SLICE_H + FOOTER_H, channels: 4, background: '#0b0b16' },
})
  .composite([
