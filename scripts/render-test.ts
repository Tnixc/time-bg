...
const rampRow = await row(
	await Promise.all(
		sunsetOffsets.map((off) => {
			const c = getSkyColors(times, 20 * 60 + off);
			console.log(
				`sunset${off >= 0 ? "+" : ""}${off}`.padEnd(14),
				JSON.stringify(c),
			);
			return generatePng(c, W, H);
		}),
...
const cols = 6;
const out = sharp({
	create: { width: W * cols, height: H * 4, channels: 3, background: "#000" },
}).composite([
...
console.log(
	"row1: phases  row2: day-walk  row3: [sunset sRGB | OKLCH | day sRGB | OKLCH]  row4: sunset ramp -100..+45min",
);

await sharp(
	await generatePng(getSkyColors(times, 19 * 60 + 50), 1920, 1080),
).toFile("/tmp/sky-landscape.png");
